...
export async function POST(request) {
  try {
    ...
    // Create request data with PROPER DATE HANDLING - STORING AS STRINGS
    const requestData = {
      type: 'New Customer',
      customerName: name,
      customerNumber: normalizedCustomerNumber,
      ...
      step2Data: {
        ...
        // CRITICAL FIX: Store dates as STRINGS (YYYY-MM-DD) not Date objects
        loanDate: loanSelectionType === 'single' ? loanDateInput : null, // Store as string
        emiStartDate: loanSelectionType === 'single' ? emiStartDateInput : null, // Store as string
    ...
    console.log('📋 Creating new customer request with dates:', {
      ...
      businessName,
      loanSelectionType,
      // FIXED: loanDate is now a string, not a Date object
      loanDateUTC: requestData.step2Data.loanDate || 'N/A', // ✅ Just log the string
      emiStartDateUTC: requestData.step2Data.emiStartDate || 'N/A', // ✅ Just log the string

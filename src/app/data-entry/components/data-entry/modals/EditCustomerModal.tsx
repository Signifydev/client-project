...
export default function EditLoanModal({
  ...
  // Calculate total loan amount and validate
  useEffect(() => {
    calculateTotalLoanAmount();
    validateLoanAmount();
  }, [formData.amount, formData.emiAmount, formData.loanDays, formData.emiType, formData.customEmiAmount, formData.loanType]);
  ...
  const validateLoanAmount = () => {
    ...
    const calculatedTotal = totalLoanAmount;
    
    if (enteredAmount > 0 && calculatedTotal > 0 && Math.abs(enteredAmount - calculatedTotal) > 0.01) {
      setLoanAmountError(`Amount mismatch! Calculated total is ₹${calculatedTotal.toLocaleString()}. Make sure the loan amount matches the EMI calculation.`);
    } else {
      setLoanAmountError('');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    // Validate loan amount matches calculation
    const enteredAmount = parseFloat(formData.amount);
    const calculatedTotal = totalLoanAmount;
    
    if (Math.abs(enteredAmount - calculatedTotal) > 0.01) {
      const confirm = window.confirm(
        `Loan amount (₹${enteredAmount.toLocaleString()}) doesn't match calculated total (₹${calculatedTotal.toLocaleString()}).\n\nDo you want to continue anyway?`
      );
      if (!confirm) return;
    }
          ...
          <div className="bg-white shadow-sm rounded-xl border border-gray-200 mb-8">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {/* Loan Amount (Editable) - UPDATED LABEL */}
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="number"
                      className={`w-full pl-12 px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200 ${
                        loanAmountError ? 'border-red-300' : 'border-gray-300'
                      }`}
                      value={formData.amount}
                      onChange={(e) => setFormData(prev => ({ ...prev, amount: e.target.value }))}
                      min="0"
                      step="0.01"
                      placeholder="Enter amount (principal)"
                    />
                  </div>
                  {loanAmountError && (

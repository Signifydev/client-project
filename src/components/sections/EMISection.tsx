...
import { useEMI, fetchCustomerLoans } from '@/src/hooks/useEMI';
  ...
  const { totalLoans, totalEMIAmount } = useMemo(() => {
    ...
    if (customerLoans && customerLoans.length > 0) {
      return {
        ...
        totalEMIAmount: customerLoans.reduce((sum, loan) => sum + (loan.emiAmount || 0), 0)
      };
    }
            ...
            <div className="space-y-3">
              ...
              {customerLoans.map((loan) => {
                const lastEmiDate = loan.lastEmiDate 
                  ? new Date(loan.lastEmiDate).toLocaleDateString()
                  : 'No EMI paid';
                
                const isLoanActive = loan.status === 'active';
                const canPay = isLoanActive && (loan.remainingAmount || 0) > 0;

                return (
                  <div key={loan._id} className="bg-white p-4 rounded-lg border border-gray-200">
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-100 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                      ...
                      <div>
                        ...
                        <span className="ml-1 font-medium text-red-600">
                          ₹{(loan.remainingAmount || loan.amount || 0).toLocaleString()}
                        </span>
                      </div>
                    </div>
                  </div>
...
const EMISection: React.FC<EMISectionProps> = React.memo(({
  ...
  const fetchCustomerLoansData = useCallback(async (customerId: string) => {
    ...
    try {
      ...
      setCustomerLoans(prev => ({
        ...prev,
        [customerId]: { loans: loans || [], loading: false }
      }));
    } catch (error) {

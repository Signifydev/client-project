 ...
 * @param {string} lastScheduledEmiDate - Last EMI date in YYYY-MM-DD
 * @param {string} loanType - Type of loan (Daily, Weekly, Monthly)
 * @param {string} emiStartDate - EMI start date in YYYY-MM-DD
 * @param {number} emiPaidCount - Number of EMIs paid
 * @param {number} totalEmiCount - Total number of EMIs
 * @returns {string} Next scheduled EMI date in YYYY-MM-DD
 */
 ...
 * @param {string} emiStartDate - EMI start date in YYYY-MM-DD
 * @param {string} loanType - Type of loan (Daily, Weekly, Monthly)
 * @param {number} totalEmisPaid - Total number of EMIs paid
 * @returns {string} Last scheduled EMI date in YYYY-MM-DD
 */
...
emiPaymentSchema.pre('save', async function() {
  ...
  if (this.isNew && this.status === 'Partial' && !this.partialChainId) {
    ...
    // ✅ CRITICAL FIX #1: Set installmentTotalAmount to FULL EMI amount
    // If originalEmiAmount is provided (from frontend), use it
    // Otherwise, we'll try to fetch it from the loan or use amount as fallback
    if (this.originalEmiAmount && this.originalEmiAmount > this.amount) {
      this.installmentTotalAmount = this.originalEmiAmount;
      console.log('✅ Using provided originalEmiAmount for partial payment:', this.originalEmiAmount);
    } else {
      // Temporary - will be fixed in post-save hook
      this.installmentTotalAmount = this.amount;
      console.log('⚠️ Using payment amount as temporary installmentTotalAmount:', this.amount);
    }
    ...
    console.log('🔧 Partial payment chain created:', {
      ...
      note: this.originalEmiAmount ? 'Has original EMI amount' : 'Missing original EMI amount - will fetch from loan'
    });
  }
  ...
  if (!this.originalEmiAmount) {
    ...
    if (this.chainParentId) {
      try {
        ...
        if (parentPayment && parentPayment.originalEmiAmount) {
          ...
          console.log('✅ Inherited originalEmiAmount from parent:', this.originalEmiAmount);
        }
      } catch (error) {
    ...
    // If still not set, use installmentTotalAmount if it's different from amount (for partial payments)
    if (!this.originalEmiAmount && this.installmentTotalAmount && this.installmentTotalAmount !== this.amount) {
      ...
      console.log('✅ Set originalEmiAmount from installmentTotalAmount:', this.originalEmiAmount);
    }
    
    // Last resort: use the current amount (for non-partial payments)
    if (!this.originalEmiAmount) {
      this.originalEmiAmount = this.amount;
      console.log('⚠️ Using payment amount as originalEmiAmount (fallback):', this.originalEmiAmount);
    }
  }
  
  // ✅ FIXED: Ensure installmentTotalAmount is set to originalEmiAmount for partial payments
  if (this.status === 'Partial' && this.originalEmiAmount && this.originalEmiAmount !== this.installmentTotalAmount) {
    this.installmentTotalAmount = this.originalEmiAmount;
    console.log('✅ Updated installmentTotalAmount to match originalEmiAmount:', this.installmentTotalAmount);
  }
});

// ==============================================
// ✅ NEW: POST-SAVE HOOK TO FETCH CORRECT EMI AMOUNT
// ==============================================
emiPaymentSchema.post('save', async function(doc) {
  try {
    // ✅ FIXED: If this is a partial payment and originalEmiAmount is missing or incorrect
    if (doc.status === 'Partial' && (!doc.originalEmiAmount || doc.originalEmiAmount <= doc.amount)) {
      // Fetch the loan to get correct EMI amount
      const loan = await mongoose.model('Loan').findById(doc.loanId);
      if (loan) {
        // Calculate which installment this is
        const currentInstallment = (loan.emiPaidCount || 0) + 1;
        ...
        if (loan.emiType === 'custom' && loan.loanType !== 'Daily') {
          if (currentInstallment === loan.totalEmiCount) {
            correctEmiAmount = loan.customEmiAmount || loan.emiAmount || 0;
          }
        }
        
        // Only update if we got a valid EMI amount that's different
        if (correctEmiAmount > 0 && correctEmiAmount !== doc.originalEmiAmount) {
          // Update the payment with correct EMI amount
          ...
          console.log('✅ Fixed partial payment EMI amount in post-save:', {
            paymentId: doc._id,
    ...
    // ✅ FIXED: If this payment has children, update their chain info
    ...
    // ✅ FIXED: If this payment has a parent, update parent's children list
...
// ✅ NEW: Virtual for isPartialChainComplete - checks if full EMI is paid
...
emiPaymentSchema.methods.addPartialPayment = async function(amount, paymentDate, collectedBy, notes = '') {
  ...
  const newPayment = new EMIPayment({
    ...
    installmentTotalAmount: this.installmentTotalAmount,
    originalEmiAmount: this.originalEmiAmount || this.installmentTotalAmount || this.amount,
    chainSequence: (this.chainChildrenIds?.length || 0) + 2
  });
...
emiPaymentSchema.methods.completePartialChain = async function(completionAmount, paymentDate, collectedBy, notes = '') {
  ...
  // Get current chain totals
  const chainPayments = await this.constructor.find({ partialChainId: this.partialChainId });
  const totalPaid = chainPayments.reduce((sum, p) => sum + p.amount, 0);
  const remainingAmount = (this.installmentTotalAmount || this.originalEmiAmount || this.amount) - totalPaid;
  ...
  const updatedChain = await this.constructor.updateChainTotals(this.partialChainId);
  
  // If chain is now complete, update all payments to Paid status
  if (updatedChain?.isComplete) {
    await EMIPayment.updateMany(
      { partialChainId: this.partialChainId },
      { $set: { status: 'Paid', isChainComplete: true } }
    );
  }
  ...
  return {
    ...
    remainingAmount: Math.max(0, remainingAmount - completionAmount)
  };
};
...
emiPaymentSchema.statics.getPaymentsInChain = async function(chainId) {
  return this.find({ partialChainId: chainId })
    ...
    .populate('chainParentId', 'amount status paymentDate originalEmiAmount')
    .populate('chainChildrenIds', 'amount status paymentDate originalEmiAmount');
};
...
emiPaymentSchema.statics.updateChainTotals = async function(chainId) {
  ...
  // ✅ CRITICAL FIX: Use originalEmiAmount if available, otherwise use installmentTotalAmount or amount
  ...
  // Update all payments in the chain with correct amounts
  ...
  console.log('🔗 Chain totals updated:', {
    chainId,
    installmentTotalAmount,
...
emiPaymentSchema.statics.completePartialPayment = async function(parentPaymentId, additionalAmount, paymentDate, collectedBy, notes = '') {
  ...
  try {
    ...
    // Calculate remaining amount using FULL EMI amount
    const chainInfo = await this.getChainSummary(parentPayment.partialChainId || parentPayment._id.toString());
    if (!chainInfo) {
      throw new Error('Could not retrieve chain information');
    }
...
emiPaymentSchema.statics.getChainSummary = async function(chainId) {
  ...
  return {
    ...
    remainingAmount: Math.max(0, fullEmiAmount - totalPaid),
...
emiPaymentSchema.statics.syncWithLoanHistory = async function(paymentId) {
  try {
    ...
    const paymentData = {
      ...
      // ✅ NEW: Include original EMI amount
      originalEmiAmount: payment.originalEmiAmount,
      // ✅ NEW: Include chain info

...
emiPaymentSchema.statics.completePartialPayment = async function(partialPaymentId, additionalAmount, completionDate, collectedBy, notes = '') {
  return SafeSession.withTransaction('Complete Partial Payment', async (session) => {
    ...
    if (isNowFullyPaid) {
      ...
      // ✅ Use Loan.js method to update counts (will increment emiPaidCount)
      await loan.updateEMIPayment(
        additionalAmount, // Only the additional amount for this payment
        completionDate || getCurrentDateString(),
        collectedBy,
        `Completion of partial payment ${partialPaymentId}: ${notes}`,
        'single',
        originalAmount
      );
    } else {
 ...
 * ✅ UPDATED: Aligned with Loan.js logic
 */
emiPaymentSchema.statics.createAdvancePayments = async function(advanceData) {
  ...
  // ✅ FIXED: Check if loan can accept advance payments
  if (loan.status === 'completed' || loan.isRenewed) {
  ...
  const payments = paymentDates.map((item, index) => ({
    customerId,
    customerName,
    customerNumber: loan.customerNumber,
    loanId,
    ...
    notes: `Advance payment ${index + 1} of ${paymentDates.length}`
  }));
  
  // Use bulk insert for efficiency
  const createdPayments = await this.insertMany(payments, { ordered: false });
  
  // ✅ FIXED: Update loan using Loan.js method for consistency
  for (const payment of createdPayments) {
    await loan.updateEMIPayment(
      ...
      payment.notes,
      'advance',
      loan.getInstallmentAmount(payment.installmentNumber)
    );
  }
  ...
  return {
    ...
    totalAmount,
    totalInstallments: paymentDates.length
  };
};

/**
 * ✅ ENHANCED: Find partial payments for a loan that can be completed
 */

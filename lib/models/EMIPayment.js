...
// ==============================================
// ✅ UPDATED: STATIC METHODS FOR MANUAL CONTROL
// ==============================================

// ✅ UPDATED: Complete partial payment WITHOUT validation
emiPaymentSchema.statics.completePartialPayment = async function(
  ...
  try {
    ...
    // ✅ FIXED: Get chain info for reference only (not for validation)
    ...
    console.log('🔨 Completing partial payment (Manual Control):', {
      parentPaymentId,
      additionalAmount,
      fullEmiAmount,
      totalPaidSoFar,
    ...
    console.log('✅ Partial payment completed (Manual Control):', {
      parentPaymentId,
    ...
    return {
      ...
      details: {
        ...
        suggestedRemainingBefore: suggestedRemaining,
        suggestedRemainingAfter: Math.max(0, suggestedRemaining - additionalAmount),
        isChainComplete: updatedChain?.isComplete || false
      }
    };
...
// ✅ NEW: Edit payment with manual control
emiPaymentSchema.statics.editPaymentWithManualControl = async function(paymentId, updates) {
  ...
  try {
    ...
    const payment = await this.findById(paymentId).session(session);
    if (!payment) {
    ...
    // ✅ Apply updates with NO validation
    ...
    console.log('✅ Payment edited (Manual Control):', {
      paymentId,
      changes: {

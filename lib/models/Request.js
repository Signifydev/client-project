...
const requestSchema = new mongoose.Schema({
  ...
  customerId: {
    ...
    ref: 'Customer',
    required: function() {
      // Required for all types except New Customer
  ...
  // NEW: Store multi-step form data for New Customer requests
  step1Data: {
    type: {
      name: String,
      phone: [String],
  ...
  // Store the current data (before changes) - for edit requests
  currentData: {
    type: mongoose.Schema.Types.Mixed,
    default: null
  },
  
  // Store the requested changes - for edit requests
  requestedData: {
    type: mongoose.Schema.Types.Mixed,
    required: function() {
      // Not required for New Customer requests that use step data
      return this.type !== 'New Customer';
    }
  },
...
// Update the updatedAt field before saving and normalize status
...
requestSchema.methods.generateAutoDescription = function() {
  switch(this.type) {
    case 'New Customer':
      return `New customer registration for ${this.customerName} - Customer Number: ${this.step1Data?.customerNumber || 'N/A'}`;
    case 'New Loan':
...
requestSchema.methods.validateStepData = function() {
  ...
  if (this.type === 'New Customer') {
    ...
    if (!this.step1Data) {
      ...
      if (!this.step1Data.name?.trim()) errors.push('Customer name is required');
      if (!this.step1Data.phone?.length || !this.step1Data.phone[0]?.trim()) errors.push('Primary phone number is required');
      if (!this.step1Data.businessName?.trim()) errors.push('Business name is required');
      if (!this.step1Data.area?.trim()) errors.push('Area is required');
      if (!this.step1Data.customerNumber?.trim()) errors.push('Customer number is required');
      if (!this.step1Data.address?.trim()) errors.push('Address is required');
    ...
    if (!this.step2Data) {
      ...
      if (!this.step2Data.loanType) errors.push('Loan type is required');
    ...
    if (!this.step3Data) {
      ...
      if (!this.step3Data.loginId?.trim()) errors.push('Login ID is required');
      if (!this.step3Data.password?.trim()) errors.push('Password is required');

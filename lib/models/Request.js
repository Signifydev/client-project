...
const requestSchema = new mongoose.Schema({
  ...
  // ============================================================================
  // UPDATED: Step 1 - Document references added for New Customer requests
  // ============================================================================
  step1Data: {
    name: { 
      type: String, 
    ...
    // OLD STRUCTURE: Legacy document objects (for backward compatibility)
    profilePicture: {
    ...
    // NEW: Document references (for new documents uploaded via Document model)
    profilePictureRef: {
      type: mongoose.Schema.Types.ObjectId,
      ref: 'Document',
      default: null
    ...
    // Metadata flags (for backward compatibility)
    ...
    // NEW: Cloudinary URLs for direct storage (simplified approach)
    cloudinaryUrls: {
      profilePicture: { type: String, default: null },
      fiDocumentShop: { type: String, default: null },
      fiDocumentHome: { type: String, default: null }
    }
  },
...
// ============================================================================
// NEW INDEXES FOR DOCUMENT REFERENCES
// ============================================================================
requestSchema.index({ 'step1Data.profilePictureRef': 1 });
requestSchema.index({ 'step1Data.fiDocumentRefs.shop': 1 });
requestSchema.index({ 'step1Data.fiDocumentRefs.home': 1 });
...
requestSchema.pre('save', function(next) {
  ...
  if (this.type !== 'New Customer') {
    ...
    // NEW: Sync document metadata flags for step1Data
    if (this.step1Data) {
      // Set hasProfilePicture flag based on any available source
      this.step1Data.hasProfilePicture = !!(
        this.step1Data.profilePictureRef ||
        this.step1Data.profilePicture?.url ||
        this.step1Data.cloudinaryUrls?.profilePicture
      );
      
      // Set hasFiDocuments flags
      this.step1Data.hasFiDocuments = {
        shop: !!(
          this.step1Data.fiDocumentRefs?.shop ||
          this.step1Data.fiDocuments?.shop?.url ||
          this.step1Data.cloudinaryUrls?.fiDocumentShop
        ),
        home: !!(
          this.step1Data.fiDocumentRefs?.home ||
          this.step1Data.fiDocuments?.home?.url ||
          this.step1Data.cloudinaryUrls?.fiDocumentHome
        )
      };
    }
  }
...
// ============================================================================
// NEW VIRTUAL PROPERTIES FOR DOCUMENT ACCESS
// ============================================================================

// Virtual for getting profile picture URL from any source
requestSchema.virtual('profilePictureUrl').get(function() {
  if (!this.step1Data) return null;
  
  // Priority 1: Document reference (if populated)
  if (this.step1Data.profilePictureRef && typeof this.step1Data.profilePictureRef === 'object') {
    return this.step1Data.profilePictureRef.url;
  }
  
  // Priority 2: Cloudinary URL (simplified storage)
  if (this.step1Data.cloudinaryUrls?.profilePicture) {
    return this.step1Data.cloudinaryUrls.profilePicture;
  }
  
  // Priority 3: Legacy structure
  if (this.step1Data.profilePicture?.url) {
    return this.step1Data.profilePicture.url;
  }
  
  return null;
});

// Virtual for getting FI document URLs
requestSchema.virtual('fiDocumentUrls').get(function() {
  if (!this.step1Data) return { shop: null, home: null };
  
  const result = { shop: null, home: null };
  
  // Helper function to get URL from any source
  const getDocumentUrl = (type) => {
    // Check Document reference
    const ref = this.step1Data.fiDocumentRefs?.[type];
    if (ref && typeof ref === 'object') {
      return ref.url;
    }
    
    // Check Cloudinary URL
    const cloudinaryKey = type === 'shop' ? 'fiDocumentShop' : 'fiDocumentHome';
    if (this.step1Data.cloudinaryUrls?.[cloudinaryKey]) {
      return this.step1Data.cloudinaryUrls[cloudinaryKey];
    }
    
    // Check legacy structure
    const legacy = this.step1Data.fiDocuments?.[type];
    if (legacy?.url) {
      return legacy.url;
    }
  ...
  result.shop = getDocumentUrl('shop');
  result.home = getDocumentUrl('home');
  
  return result;
});

// Virtual for getting all document URLs in a flat structure
requestSchema.virtual('allDocumentUrls').get(function() {
  return {
    profilePicture: this.profilePictureUrl,
    fiDocumentShop: this.fiDocumentUrls.shop,
    fiDocumentHome: this.fiDocumentUrls.home
  };
});

// Virtual for checking if request has documents
requestSchema.virtual('hasDocuments').get(function() {
  const urls = this.allDocumentUrls;
  return !!(urls.profilePicture || urls.fiDocumentShop || urls.fiDocumentHome);
});

// ============================================================================
// EXISTING VIRTUAL PROPERTIES (UNCHANGED)
// ============================================================================
...
// Method to add document reference to request
requestSchema.methods.addDocumentReference = function(documentId, documentType) {
  if (!this.step1Data) {
    this.step1Data = {};
  }
  ...
  switch(documentType) {
    case 'profile_picture':
      this.step1Data.profilePictureRef = documentId;
      break;
    case 'fi_document_shop':
      if (!this.step1Data.fiDocumentRefs) {
        this.step1Data.fiDocumentRefs = {};
      }
      this.step1Data.fiDocumentRefs.shop = documentId;
      break;
    case 'fi_document_home':
      if (!this.step1Data.fiDocumentRefs) {
        this.step1Data.fiDocumentRefs = {};
      }
      this.step1Data.fiDocumentRefs.home = documentId;
      break;
    default:
      return false;
  }
...
// Method to add Cloudinary URL directly (simplified approach)
requestSchema.methods.addCloudinaryUrl = function(url, documentType) {
  if (!this.step1Data) {
    this.step1Data = {};
  }
  
  if (!this.step1Data.cloudinaryUrls) {
    this.step1Data.cloudinaryUrls = {};
  }
  
  switch(documentType) {
    case 'profile_picture':
      this.step1Data.cloudinaryUrls.profilePicture = url;
      break;
    case 'fi_document_shop':
      this.step1Data.cloudinaryUrls.fiDocumentShop = url;
      break;
    case 'fi_document_home':
      this.step1Data.cloudinaryUrls.fiDocumentHome = url;
      break;
    default:
      return false;
  }
...
requestSchema.methods.getRequestSummary = function() {
  const summary = {
    ...
    // NEW: Add document info
    hasDocuments: this.hasDocuments,
    documentUrls: this.allDocumentUrls
  };

  ...
  if (this.type !== 'New Customer') {
    ...
    if (this.step2Data) {
      ...
      if (this.step2Data.loanSelectionType === 'multiple') {
        ...
        // For single loans, store input dates as strings
        if (this.step2Data.loanDate && this.step2Data.loanDate instanceof Date) {
          this.step2Data.loanDateInput = this.step2Data.loanDate.toISOString().split('T')[0];
        }
        if (this.step2Data.emiStartDate && this.step2Data.emiStartDate instanceof Date) {
          this.step2Data.emiStartDateInput = this.step2Data.emiStartDate.toISOString().split('T')[0];
        }
      }
    }
  }
...
// Virtual for loan dates in DD/MM/YYYY format
...
// Virtual for loan summary with display dates
requestSchema.virtual('loanSummary').get(function() {
  if (this.step2Data && this.step2Data.loanSelectionType === 'single') {
    ...
    return {
      ...
      loanDateInput: this.step2Data.loanDateInput,
      emiStartDateInput: this.step2Data.emiStartDateInput
    };
  }
  return null;
});
...
// Validate step data method - FIXED to check both amount and loanAmount with IST date validation
  ...
  if (this.type === 'New Customer') {
    ...
    if (!this.step2Data) {
      ...
      if (this.step2Data.loanSelectionType === 'single') {
        ...
        // Validate dates in IST timezone
        if (this.step2Data.emiStartDate && this.step2Data.loanDate) {
          const emiStartDateIST = convertUTCToIST(new Date(this.step2Data.emiStartDate));
          const loanDateIST = convertUTCToIST(new Date(this.step2Data.loanDate));
...
// Transform to include display dates in JSON/object output

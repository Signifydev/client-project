...
// Method to check if loan is overdue (using string dates)
LoanSchema.methods.isOverdue = function() {
  if (this.status !== 'active' || this.emiPaidCount >= this.totalEmiCount) {
    return false;
  }
  
  const today = getCurrentDateString();
  return compareDateStrings(today, this.nextEmiDate) > 0;
};

// Method to calculate next due date (using string dates)
LoanSchema.methods.getNextDueDate = function() {
  ...
  switch (this.loanType) {
    case 'Daily':
      nextDue = addDays(this.lastEmiDate, 1);
      break;
    case 'Weekly':
      nextDue = addDays(this.lastEmiDate, 7);
      break;
    case 'Monthly':
      nextDue = addMonths(this.lastEmiDate, 1);
      break;
    default:
      nextDue = addDays(this.lastEmiDate, 1);
  }
...
// Method to update EMI payment (using string dates) - ✅ FIXED VERSION
LoanSchema.methods.updateEMIPayment = function(amountPaid, paymentDate, collectedBy = 'system', notes = '', paymentType = 'single') {
  ...
  const payment = {
    ...
    amount: amountPaid,
    status: paymentStatus, // ✅ Uses correct status
    collectedBy,
    notes,
    loanId: this._id,
  ...
  // Update status if completed (only count FULL payments)
  ...
  // ✅ NEW: Log payment details for debugging
...
LoanSchema.methods.validateEMIConfig = function() {
  ...
  if (!isValidYYYYMMDD(this.dateApplied)) {
    errors.push('Invalid loan date format');
  }
  
  if (!isValidYYYYMMDD(this.emiStartDate)) {
    errors.push('Invalid EMI start date format');
  }
  
  if (this.nextEmiDate && !isValidYYYYMMDD(this.nextEmiDate)) {
    errors.push('Invalid next EMI date format');
  }
...
LoanSchema.statics.renewLoan = async function(originalLoanId, newLoanData, requestedBy = 'system') {
  ...
  try {
    ...
    // 3. Parse dates as strings
    const renewalDate = newLoanData.renewalDate || getCurrentDateString();
    const emiStartDate = newLoanData.emiStartDate || renewalDate;
    
    // 4. Create new loan with string dates
    const newLoan = new this({
      ...
      dateApplied: renewalDate, // String date
      ...
      emiStartDate: emiStartDate, // String date
      status: 'active',
      createdBy: requestedBy,
      originalLoanNumber: originalLoan.loanNumber,
      nextEmiDate: emiStartDate // String date
    });
    
    // Generate EMI schedule for the new loan
    newLoan.generateEMISchedule();
    
    await newLoan.save({ session, validateBeforeSave: false });
    
    // 5. Mark original loan as renewed
    originalLoan.isRenewed = true;
    originalLoan.status = 'renewed';
    originalLoan.renewedLoanNumber = newLoanNumber;
    originalLoan.renewedDate = getCurrentDateString();
...
LoanSchema.pre('save', function(next) {
  try {
    ...
    if (this.isModified('emiStartDate') || !this.nextEmiDate) {
      ...
      // When a payment is made, calculate next EMI date based on last EMI date
      this.nextEmiDate = this.getNextDueDate();
    }

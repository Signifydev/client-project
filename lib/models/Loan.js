...
const LoanSchema = new mongoose.Schema(
  {
    ...
    // CHANGED: loanAmount → amount to match API
    amount: {
      type: Number,
      required: true,
      min: 0
    ...
    // Status field - make sure it matches what your API expects
    status: {
      type: String,
      enum: ['active', 'completed', 'overdue', 'pending', 'closed', 'defaulted'],
      default: 'pending', // Changed default to 'pending' to match your API
    },
    
    // Add isMainLoan field that your API is using
    isMainLoan: {
      type: Boolean,
      default: false
    },
    ...
    totalPending: {
      type: Number,
      default: function() {
        return (this.amount || 0) - (this.totalPaid || 0); // Updated to use 'amount'
      }
    },
...
LoanSchema.pre('save', function(next) {
  ...
  this.amount = this.amount || 0; // Updated to use 'amount'
  ...
  if (this.isModified('totalPaid')) {
    this.totalPending = (this.amount || 0) - (this.totalPaid || 0); // Updated to use 'amount'
  }

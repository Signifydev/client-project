...
const LoanSchema = new mongoose.Schema(
  ...
  validate: {
    validator: function(v) {
      ...
      const emiStart = new Date(this.emiStartDate);
      const nextEmi = new Date(v);
      return nextEmi >= emiStart;
    },
...
// Method to calculate next due date (FIXED: Proper calculation based on payments)
...
LoanSchema.methods.updateEMIPayment = function(amountPaid, paymentDate, collectedBy = 'system', notes = '', paymentType = 'single') {
  ...
  // FIXED: Update next EMI date correctly based on payments
...
LoanSchema.statics.renewLoan = async function(originalLoanId, newLoanData, requestedBy = 'system') {
  ...
  try {
    ...
    const newLoan = new this({
      ...
      // FIXED: For new renewed loan, nextEmiDate should be emiStartDate (not emiStartDate + period)
      nextEmiDate: new Date(newLoanData.emiStartDate || newLoanData.renewalDate)
    });
...
LoanSchema.pre('save', function(next) {
  try {
    ...
    if (this.isModified('emiStartDate') || !this.nextEmiDate) {
      ...
      this.nextEmiDate = new Date(this.lastEmiDate);
      switch(this.loanType) {
        case 'Daily':

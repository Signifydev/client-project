...
// ✅ NEW: Method to process bulk advance payments
LoanSchema.methods.processBulkAdvancePayments = function(payments, fromDate, toDate) {
  if (this.isRenewed || this.status === 'renewed') {
    throw new Error('Cannot process payments for a renewed loan');
  }
  
  const totalAmount = payments.reduce((sum, payment) => sum + payment.amount, 0);
  const paymentCount = payments.length;
  
  console.log('🔄 Processing bulk advance payments:', {
    loanNumber: this.loanNumber,
    paymentCount,
    totalAmount,
    fromDate,
  ...
  this.nextEmiDate = formatToYYYYMMDD(nextDueDate);
  ...
  // Add payments to emiHistory
  payments.forEach(payment => {
    this.emiHistory.push({
      paymentDate: payment.date,
      amount: payment.amount,
      status: 'Advance',
      collectedBy: payment.collectedBy || 'Operator',
      notes: payment.notes || `Advance payment for ${payment.date}`,
      loanId: this._id,
      customerNumber: this.customerNumber,
      loanNumber: this.loanNumber,
      paymentType: 'advance',
  ...
  console.log('✅ Bulk advance payments processed:', {
    loanNumber: this.loanNumber,
    paymentCount,
    totalAmount,
    emiPaidCountBefore: this.emiPaidCount - paymentCount,
    emiPaidCountAfter: this.emiPaidCount,
    nextEmiDate: this.nextEmiDate,
    status: this.status
  });
